...
final class DemoSet extends Commands
{
	...
	public function __construct(private readonly string $host = 'demo.local')
	{
	}
	...
	{
		return CommandResult::json(['host' => $this->host, 'raw' => $this->raw]);
	}

	#[Command(summary: 'Filter by number.', group: 'Read')]
	public function commandContracts(
		#[Opt('Contract number.', placeholder: 'nr')] ?string $nr = null,
	): CommandResult {
		return CommandResult::json(['nr' => $nr]);
	}
	...
	public function commandEvents(
		#[Opt('MSA numbers.', placeholder: 'msaNr')] StringList $msa,
		#[Opt('Numeric ids.', placeholder: 'id')] ?IntList $ids = null,
		#[Opt('Colours.')] ?NoteTypeList $colours = null,
	): CommandResult {
		return CommandResult::json([
			'msa' => $msa->all(),
			'ids' => $ids?->all(),
			'colours' => $colours === null ? null : array_map(static fn(Colour $c): string => $c->value, $colours->all()),
		]);
	}

	#[Command(summary: 'Typed values.', group: 'Read')]
	public function commandTyped(
		#[Opt('A colour.')] Colour $colour,
		#[Opt('A count.', min: 1, max: 10)] int $count = 1,
		#[Opt('A moment.', placeholder: 'ATOM')] ?DateTimeImmutable $at = null,
		#[Opt('A date.', placeholder: 'Y-m-d', pattern: '/^\d{4}-\d{2}-\d{2}$/', hint: 'No other format is accepted.')] ?string $on = null,
		#[Opt('Free text.', allowEmpty: true, trim: false)] string $note = '',
	): CommandResult {
		return CommandResult::json([
			'colour' => $colour->value,
			'count' => $count,
	...
	public function commandReport(
		#[Opt('Exit with this instead of 0.', placeholder: 'n')] int $code = 0,
	): CommandResult {
	...
	public function commandSave(
		...
		return CommandResult::json(['saved' => $what])
	...
	{
		return CommandResult::json(['command' => $this->invocation()->name]);
	}
